...
class class_operativa:
    def __init__(self, fecha, archivo_guia, config_file):
        self.archivo_guia = archivo_guia
    ...
    def get_precip_data(self):
        ...
        actuales, conteos, f_repo = extraer_precipitacion(df, self.fecha_ini_pp, self.fecha_final, self.config_file)
    ...
    def get_temp_data(self):
        df = pd.read_excel(self.archivo_guia)
        actuales, tx_fecha, tx_repo = extraer_temp(df, self.fecha_ini_t, self.fecha_final, self.config_file)
        self.tx = actuales
        self.tx_uact = tx_repo
        self.tx_fecha = tx_fecha
        actuales, tm_fecha, tm_repo = extraer_temp(df, self.fecha_ini_t, self.fecha_final, self.config_file, 'tmin')
        self.tm = actuales
