...
def precipitacion_plot_v2(fecha, estacion, datos, cdatos, dest, archivo_historico):
...
def stat_temp_data(fecha, estacion, archivo_historico):
...
def historical_date(estacion, archivo_historico):
...
def temp_plot(fecha, estacion, x1, tx, x2, tm, dest, archivo_historico):
    xf, tx_inf, tx_sup, tm_inf, tm_sup = stat_temp_data(fecha, estacion, archivo_historico)
    lim_plot = [xf[0] - relativedelta(months=1), xf[-1] + relativedelta(months=1)]
    lb1, lb2 = historical_date(estacion, archivo_historico)
    # Datos Interpolados
    data = ma.getdata(tx)
    mask = ma.getmask(tx)
