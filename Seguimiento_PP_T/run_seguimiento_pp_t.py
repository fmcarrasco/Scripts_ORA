...
archivo_guia = './datos/Estaciones_Seguimiento_PP_T.xlsx'
archivo_historico = './datos/DatosHistoricos.xlsx'
config_file = './config_database.txt'
...
def get_data_estacion(nest):
    dest = pd.read_excel(archivo_guia)
...
a = class_operativa(fecha, archivo_guia, config_file)
...
for estacion in a.temp_estaciones:
    ...
    dest['prov'] = prov
    dest['tipo'] = tipo
    fig, ax = temp_plot(f_dt, estacion, tx_fecha, tx, tm_fecha, tm, dest, archivo_historico)
...
for estacion in a.pp_estaciones:
    ...
    dest['prov'] = prov
    dest['tipo'] = tipo
    fig, ax = precipitacion_plot_v2(a.fecha_final, estacion, datos, cdatos, dest, archivo_historico)
